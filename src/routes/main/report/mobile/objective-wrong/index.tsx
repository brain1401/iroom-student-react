...
type MobileObjectiveWrongProps = Record<string, never>;

/**
 * 객관식 오답 화면 (모바일)
 * @description 객관식 오답 상태 전용 페이지. 객관식 정답 화면과 동일한 레이아웃/동작.
 *
 * 주요 구성:
 * - 상단: 이름/반, 우측 아이콘(알림/마이/메뉴), 섹션 토글(시험 관리/성적·리포트), 제목+물음표
 * - 본문: 문제 카드, 정답/내 정답 표시(내 정답 빨간색, 숫자 '③' 강조)
 * - 하단: 해설/이론/유사 문제 추천 토글 섹션, 챗봇 버튼(시트 오픈)
 */
function MobileObjectiveWrong(_props: MobileObjectiveWrongProps) {
  ...
  return (
    <div className="flex w-full justify-center p-6">
      <div className="relative h-[804px] w-[360px] rounded-md bg-white">
        ...
        <div className="absolute left-[24px] top-[490px] text-[18px]/[1.19] font-medium text-[#FF6A71]">
          내 정답
        </div>
        <div className="absolute left-[194px] top-[486px] text-[25px]/[1.19] font-light text-[#FF6A71]">
          {wrongChoice}
        </div>

        <div className="absolute left-[24px] top-[526px] h-px w-[313px] bg-[#D7D7D7]" />
...
export const Route = createFileRoute("/main/report/mobile/objective-wrong/")({
  component: MobileObjectiveWrong,
});
